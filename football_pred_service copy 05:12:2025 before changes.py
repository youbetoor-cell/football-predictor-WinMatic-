...
def record_predictions_history(league: int, fixtures: List[Dict[str, Any]]) -> None:
    """Store model predictions in predictions_history using the new schema."""
    if not fixtures:
        return

    try:
        ensure_predictions_db()  # make sure table exists
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
        ...
        for f in fixtures:
            ...
            cur.execute(
                ...
                (
                    ...
                    away_team,
                    f.get("kickoff_utc"),
                    model_probs.get("home"),
                    model_probs.get("draw"),
                    model_probs.get("away"),
                    f.get("best_side"),
                    f.get("best_edge"),
