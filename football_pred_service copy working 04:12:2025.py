...
def ensure_predictions_db():
    """Creates the predictions_history table if it doesn't exist yet."""
...
def api_progress_metrics(
    ...
    """
    Computes accuracy and log loss for recent predictions based on the existing DB.
    """
    ensure_predictions_db()
    ...
    for (payload_json,) in rows:
        try:
            ...
            probs = payload.get("model_probs", {})
            actual = payload.get("actual_result") or None
            if not actual:
                continue
            ...
            p = max(probs.get(actual, 1e-6), 1e-6)
            log_losses.append(-math.log(p))
        except Exception:
            continue

    if not total:
        return {"ok": False, "message": "No records with actual results yet"}
