cat > scripts/calibrate_1x2.py <<'PY'
import json, math, os
...
def apply_temp_drawmult(p, T, dm):
    # temperature softening + draw multiplier
    # p is dict home/draw/away summing to 1
    eps = 1e-15
    ph = max(eps, float(p["home"]))
    pd = max(eps, float(p["draw"]))
    pa = max(eps, float(p["away"]))

    # temperature: raise to power 1/T (T>1 flattens)
    invT = 1.0 / float(T)
    ph = ph ** invT
    pd = pd ** invT
    pa = pa ** invT
...
def main():
    ...
    bt, source_url = fetch_backtest()
    sample = bt.get("sample") or []
    fixtures_scored = int(bt.get("fixtures_scored") or 0)

    # prefer RAW probs if present; fallback to model_probs
    rows = []
    for r in sample:
    ...
    print("\nTop 10 (logloss asc):")
    for (ll, neg_acc, T, dm) in fine[:10]:
        print("  ll=%.6f acc=%.4f  T=%s dm=%s  n=%d" % (ll, -neg_acc, T, dm, n))
